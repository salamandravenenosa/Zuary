// Centro de Integrações — conteúdo real
"use client";
...
const integrationConfigs = [
  ...
  {
    type: "GOOGLE_BUSINESS", name: "Google Meu Negócio",
  ...
  {
    type: "META_PIXEL", name: "Meta Pixel",
    description: "Conversões server-side via Conversions API",
    icon: Globe, color: "#7C3AED", scopes: [], oauthUrl: null,
    compliance: "Conversions API — Server-side tracking",
  },
  {
    type: "WHATSAPP_BUSINESS", name: "WhatsApp Business",
    description: "Mensagens recebidas e tempo de resposta",
    icon: MessageCircle, color: "#10B981", scopes: [], oauthUrl: null,
        ...
        {notification && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            className={cn("flex items-center gap-3 p-4 rounded-xl border",
              notification.type === "success" ? "bg-[#10B981]/10 border-[#10B981]/20 text-[#10B981]" : "bg-[#EF4444]/10 border-[#EF4444]/20 text-[#EF4444]"
            )}>
        ...
        <CardContent className="p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">Tokens encriptografados com AES-256-GCM. Nenhuma senha é coletada.</p>
            </div>
          </div>
        </CardContent>
      </Card>
          ...
          return (
            <motion.div key={config.type} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}>
              <Card className={cn("transition-all",
                hasError ? "border-[#EF4444]/20 bg-[#EF4444]/[0.03]"
                  : isConnected ? "border-[#10B981]/20 bg-[#10B981]/[0.02]"
                ...
                <CardContent className="p-5">
                  ...
                  {status?.platformName && <p className="text-xs text-muted-foreground mb-2">Perfil: {status.platformName}</p>}
